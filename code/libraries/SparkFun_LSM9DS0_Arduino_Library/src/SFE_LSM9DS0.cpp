...
void LSM9DS0::xmSetRegBit(uint8_t reg, uint8_t bit, uint8_t state) {
   uint8_t val = xmReadByte(reg);
   if (state == 0) {
       val = val & ~(0x01 << bit);
    } else {
        val = val | (0x01 << bit);
...
void LSM9DS0::setSnglclickIntEn(uint8_t state) {
        // bit 4 = Z axis
        // bit 2 = Y axis
        // bit 0 = X axis
        uint8_t val = xmReadByte(CLICK_CFG);
        if (state) { val = val | 0x15; }
        else { val & 0xEA; }
        xmWriteByte(CLICK_CFG, val);
        // set single-click enable
        xmSetRegBit(CLICK_SRC, 4, state);
}

void LSM9DS0::setClickDetThs(float ths) {
	...
	ths = (int)(((ths * 1.0) / (aRes * 1.0)) * 127);
    if ((int)ths > 0xff) {
		ths = 0xff;
	}
    xmWriteByte(CLICK_THS, ((int)ths & 0x7f));

}

void LSM9DS0::setClick(uint8_t state) {
	// set XM interrupt pins to active-high
    // XM interrupts are active-low by default
    xmSetRegBit(INT_CTRL_REG_M, 3, 1);
    
	// enable latching on interrupt 1 only
    xmSetRegBit(CTRL_REG5_XM, 0, state);
    
	// enable single-click interrupts
    setSnglclickIntEn(state);
    
    // route interrupt to XM_INT1 pin
    xmSetRegBit(CTRL_REG3_XM, 6, state);
 
	// set click detection threshold to 1.5 g
    setClickDetThs(1.5);
	
	//setClickTimeLimit to 16 ms
	xmWriteByte(TIME_LIMIT, 16 & 0xff);
}



uint8_t LSM9DS0::getIntSrc_XM(uint8_t source) {
	if (source == 1) {
		return xmReadByte(INT_GEN_1_SRC);
	} else if (source == 2) {
		return xmReadByte(INT_GEN_2_SRC);
	}
}

void LSM9DS0::setAccelInt(uint8_t state, uint8_t IntGen, uint8_t pin, uint8_t duration, float ths) {
	...
	if (pin == 1) {
		if (IntGen == 1) {
			...
			xmSetRegBit(CTRL_REG3_XM, 5, state);
		} else if (IntGen ==2) {
			// Enable Inertial Interrupt Generator 2 on INT1_XM
			xmSetRegBit(CTRL_REG3_XM, 4, state);
		}
	} else if (pin == 2) {
		if (IntGen == 1) {
			// Enable Inertial Interrupt Generator 1 on INT2_XM
			xmSetRegBit(CTRL_REG4_XM, 6, state);
		} else if (IntGen ==2) {
			// Enable Inertial Interrupt Generator 2 on INT2_XM
			xmSetRegBit(CTRL_REG4_XM, 5, state);
		}
	}
	setInertIntAxesEn(state, pin);
	...
	// Enable / Disable Interrupt Latching on INT_GEN_1 Pin
    // clear interrupts by reading INT_GEN_1_SRC
	if (IntGen == 1) {
		xmSetRegBit(CTRL_REG5_XM, 0, state);
	} else if (IntGen == 2) {
		xmSetRegBit(CTRL_REG5_XM, 1, state);
	}
	
	// throw interrupt after 1 sample over threshold
	if (IntGen == 1) {
		xmWriteByte(INT_GEN_1_DURATION, duration & 0x7f);
	} else if (IntGen == 2) {
		xmWriteByte(INT_GEN_2_DURATION, duration & 0x7f);
	}
	
	// set interrupt accel level in G
	if (ths < 0) { ths = ths * -1.0; }
	ths = ((ths/aRes) * 127);
	if (ths > 0xffff) { ths = 0xffff; }
	
	if (IntGen == 1) {
		xmWriteByte(INT_GEN_1_THS, ((int)ths & 0x7f));
	} else if (IntGen == 2) {
		xmWriteByte(INT_GEN_2_THS, ((int)ths & 0x7f));
	}
...
void LSM9DS0::setInertIntAxesEn(uint8_t state, uint8_t IntGen) {
	uint8_t val;
	if (IntGen == 1) {
		val = xmReadByte(INT_GEN_1_REG);
	} else if (IntGen == 2) {
		val = xmReadByte(INT_GEN_1_REG);
	}
	// bit 5 = Z high
	...
	if (IntGen == 1) {
		xmWriteByte(INT_GEN_1_REG, val);
	} else if (IntGen == 2) {
		xmWriteByte(INT_GEN_2_REG, val);
	}
}
