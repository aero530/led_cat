...
public:
	...
	void xmSetRegBit(uint8_t reg, uint8_t bit, uint8_t state);

	void setSnglclickIntEn(uint8_t state);
	
	void setClickDetThs(float ths);

	void setClick(uint8_t state);

	uint8_t getIntSrc_XM(uint8_t);
	
	void setAccelInt(uint8_t state, uint8_t IntGen, uint8_t pin, uint8_t duration, float ths);

	
	void setInertIntAxesEn(uint8_t state, uint8_t IntGen);
...
/*
	CTRL_REG3_XM - config tap, int1, or int2 on INT1_XM pin
	0b00000000 - default
	0b01000000 - tap generator interrupt on INT1_XM pin
	0b00100000 - inertial interrupt generator 1 on INT1_XM pin
	0b00010000 - inertial interrupt generator 2 on INT1_XM pin

	CTRL_REG4_XM - config tap, int1, or int2 on INT2_XM pin
	0b00000000 - default
	0b01000000 - tap generator interrupt on INT2_XM pin
	0b00100000 - inertial interrupt generator 1 on INT2_XM pin
	0b00010000 - inertial interrupt generator 2 on INT2_XM pin
	
	INT_GEN_1_REG - settings for the inertial interrupt generator 1
	0b01111111 - detect movement
	0b11111111 - detect position
	
	INT_GEN_1_SRC -  status for the inertial interrupt generator 1
	...
	0b 0 IA ZH ZL YH YL XH YL
	IA = interrupt status
	XYZ / HL high low int for each axis
	
	INT_GEN_1_THS - threshold value (7 bit, msb is 0)
	
	INT_GEN_1_DURATION - min duration of the interrupt event to be recognized.  (7 bit, msb is 0)
	
	
	
	CLICK_CFG - enable interrupt single or double click
	0b00100000 - enable double click on z-axis
	
	CLICK_SRC - click source readout
	
	CLICK_THS - threshold setting for click (7 bit, msb is 0)
